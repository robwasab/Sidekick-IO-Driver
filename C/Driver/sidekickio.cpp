...
void SidekickIO::send_config_layout_spim(
...
void SidekickIO::send_gpio_config(
...
void SidekickIO::send_gpio_pin_set(
...
void SidekickIO::send_gpio_read(
		uint8_t gpio_index, bool * level)
{
	Packet rsp = {0};
	size_t rsplen = 0;
...
void SidekickIO::send_gpio_get_intrpt_status(uint32_t * gpio_int_mask)
{
...
void SidekickIO::send_gpio_clr_intrpt_status(uint32_t gpio_int_mask)
{
	Packet rsp = {0};
	size_t rsplen = 0;
...
void SidekickIO::send_gpio_enable_pin_intrpt(uint8_t gpio_index)
{
	Packet rsp = {0};
	size_t rsplen = 0;
...
void SidekickIO::main_loop_task(void)
{
	...
	if(INTRPT_STATUS_GPIO_Msk & status) {
		...
		send_gpio_get_intrpt_status(&gpio_int_status);

		printf("gpio interrupt: %04x!\n", gpio_int_status);

		// clear the interrupt
		send_gpio_clr_intrpt_status(gpio_int_status);
